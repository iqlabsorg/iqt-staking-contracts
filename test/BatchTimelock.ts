...
describe('BatchTimelock Contract', function () {
  ...
  beforeEach(async function () {
    ...
    batchTimelock = (await run('deploy:batch-timelock', {
  ...
  describe('Functionality', function () {
    ...
    describe('addTimelock', function () {
      beforeEach(async function () {
        ...
        await batchTimelock
          .connect(deployer)
          .addTimelock(timelockReceiver1Address, TIMELOCK_AMOUNT, TIMESTAMP_NOW, CLIFF_DURATION, VESTING_DURATION);
      });
      ...
      it('Should correctly add a timelock', async function () {
        it('Should correctly emit a TimelockCreated event', async function () {
          await expect(
            batchTimelock
              .connect(deployer)
              .addTimelock(timelockReceiver2Address, TIMELOCK_AMOUNT, TIMESTAMP_NOW, CLIFF_DURATION, VESTING_DURATION),
          )
      ...
      it('Should fail to add a timelock for a zero address', async function () {
        await expect(
          batchTimelock
            .connect(deployer)
            .addTimelock(ZERO_ADDRESS, TIMELOCK_AMOUNT, TIMESTAMP_NOW, CLIFF_DURATION, VESTING_DURATION),
      ...
      it('Should fail to add a timelock if total amount is 0', async function () {
        await expect(
          batchTimelock
            .connect(deployer)
            .addTimelock(timelockReceiver1Address, 0, TIMESTAMP_NOW, CLIFF_DURATION, VESTING_DURATION),
    ...
    describe('addTimelockBatch', function () {
      ...
      beforeEach(async function () {
        ...
        batchOfReceivers = [
          {
            receiver: timelockReceiver1Address,
            totalAmount: TIMELOCK_AMOUNT,
            timelockFrom: TIMESTAMP_NOW,
            cliffDuration: CLIFF_DURATION,
            vestingDuration: VESTING_DURATION,
          },
          {
            receiver: timelockReceiver2Address,
            totalAmount: TIMELOCK_AMOUNT,
            timelockFrom: TIMESTAMP_NOW,
            cliffDuration: CLIFF_DURATION,
            vestingDuration: VESTING_DURATION,
          ...
          {
            receiver: timelockReceiver3Address,
            totalAmount: TIMELOCK_AMOUNT,
            timelockFrom: TIMESTAMP_NOW,
            cliffDuration: CLIFF_DURATION,
            vestingDuration: VESTING_DURATION,
      ...
      it('Should fail to add a batch if the caller is not a timelock creator', async function () {
        await expect(batchTimelock.connect(stranger).addTimelockBatch(batchOfReceivers)).to.be.revertedWithCustomError(
          batchTimelock,
      ...
      it('Should fail to add a batch with a zero address', async function () {
        ...
        await expect(batchTimelock.connect(deployer).addTimelockBatch(batchOfReceivers)).to.be.revertedWithCustomError(
          batchTimelock,
      ...
      it('Should fail to add a batch for an existing receiver', async function () {
        ...
        await expect(batchTimelock.connect(deployer).addTimelockBatch(batchOfReceivers)).to.be.revertedWithCustomError(
          batchTimelock,
          `ReceiverAlreadyHasATimelock`,
        );
      });

      it('Should fail to add a batch if the batch is empty', async function () {
        await expect(batchTimelock.connect(deployer).addTimelockBatch([])).to.be.revertedWithCustomError(
          batchTimelock,
          `EmptyReceiversArray`,
        );
      });
    ...
    describe('claim function', function () {
      ...
      it('Should fail to claim when amount is zero', async function () {
        await expect(batchTimelock.connect(timelockReceiver1).claim(0)).to.be.revertedWithCustomError(
          batchTimelock,
          'ZeroClaimAmount',
        );
      });
      ...
      it('Should fail to claim before the cliff period ends', async function () {
        await expect(
          batchTimelock.connect(timelockReceiver1).claim(ethers.parseEther('0.1')),
      ...
      it('Should fail to claim more than the withdrawable amount', async function () {
        ...
        await expect(batchTimelock.connect(timelockReceiver1).claim(amountToClaim)).to.be.revertedWithCustomError(
          batchTimelock,
          'AmountExceedsWithdrawableAllowance',
        );
      });
      ...
      it('Should allow to claim only half of the amount if terminated on the half of the vesting', async function () {
        ...
        await batchTimelock
          .connect(deployer)
          .terminate(timelockReceiverAddress, timelockFrom + cliffDuration + vestingDuration / 2);
      ...
      it('Should fail if the token transfer fails', async function () {
        ...
        await ethers.provider.send('evm_increaseTime', [cliffDuration + vestingDuration + 1]);
        await ethers.provider.send('evm_mine', []);
        await expect(batchTimelock.connect(timelockReceiver1).claim(initialVestingAmount)).to.be.revertedWith(
    ...
    describe('getClaimableBalance function', function () {
      ...
      beforeEach(async function () {
        ...
        await batchTimelock
          .connect(deployer)
          .addTimelock(timelockReceiver1Address, initialVestingAmount, timelockFrom, cliffDuration, vestingDuration);
      });

      it('Should return the correct claimable balance if terminated right after start', async function () {
        await batchTimelock.connect(deployer).terminate(timelockReceiver1Address, timelockFrom + 1);

        await ethers.provider.send('evm_increaseTime', [1]);
        await ethers.provider.send('evm_mine', []);

        const claimableBalance = await batchTimelock.getClaimableBalance(timelockReceiver1Address);
        console.log('claimableBalance', claimableBalance.toString());
      ...
      it("Should not return negative vested amount after claim", async function () {
        await ethers.provider.send('evm_increaseTime', [cliffDuration + (vestingDuration / 4)]);
        await ethers.provider.send('evm_mine', []);

        let claimableBalance = await batchTimelock.getClaimableBalance(timelockReceiver1Address);
        console.log('claimableBalance', claimableBalance.toString());
        await batchTimelock.connect(timelockReceiver1).claim(claimableBalance);

        claimableBalance = await batchTimelock.getClaimableBalance(timelockReceiver1Address);
        console.log('claimableBalance after claim', claimableBalance.toString());
    ...
    describe('getTimelockReceivers', function () {
      ...
      beforeEach(async function () {
        ...
        await batchTimelock
          .connect(deployer)
          .addTimelock(timelockReceiver3Address, TIMELOCK_AMOUNT, TIMESTAMP_NOW, CLIFF_DURATION, VESTING_DURATION);
        await batchTimelock
          .connect(deployer)
          .addTimelock(timelockReceiver4Address, TIMELOCK_AMOUNT, TIMESTAMP_NOW, CLIFF_DURATION, VESTING_DURATION);
      });
    ...
    describe('terminate and determinate functions', function () {
      ...
      it('Should re-enable claiming after determinate', async function () {
        ...
        await expect(batchTimelock.connect(timelockReceiver1).claim(CLAIM_AMOUNT)).not.to.be.reverted;
      });
    });
