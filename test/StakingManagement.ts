  ...
  describe('Functionality', function () {
    ...
    describe('addStakingPlan', function () {
      ...
      it('Should not allow to add without STAKING_MANAGER role', async function () {
        await expect(
          stakingManagement.connect(stranger).addStakingPlan(ONE_MONTH_IN_SECONDS, ONE_MONTH_APY),
      ...
      it('Should not allow to add a staking plan with the duration less than 1 day', async function () {
        await expect(
          stakingManagement.addStakingPlan(ONE_DAY_IN_SECONDS - 1, ONE_MONTH_APY),
        ).to.be.revertedWithCustomError(stakingManagement, 'DurationMustBeGreaterThanOneDay');
      });

      it('Should not allow to add a staking plan with the 0% APY', async function () {
        await expect(
          stakingManagement.addStakingPlan(ONE_MONTH_IN_SECONDS, ZERO_PERCENT),
      ...
      it('Should not allow to add a staking plan with more the 100% APY', async function () {
        await expect(
          stakingManagement.addStakingPlan(ONE_MONTH_IN_SECONDS, HUNDED_ONE_PERCENT),
        ).to.be.revertedWithCustomError(stakingManagement, 'APYMustBeWithinRange');
      });
    });
    ...
    describe('removeStakingPlan', function () {
      ...
      it('Should not allow to remove a staking plan twice', async function () {
        await stakingManagement.connect(deployer).removeStakingPlan(planId);
        await expect(stakingManagement.removeStakingPlan(planId)).to.be.revertedWithCustomError(
          stakingManagement,
      ...
      it('Should not allow to remove a staking plan if there are active stakes', async function () {
        ...
        const requiredStakingPoolBalance =
          BigInt(STAKING_AMOUNT) * BigInt(2) +
          BigInt(stake1EstimatedEarnings.predictedEarningsInTokens) +
          BigInt(stake2EstimatedEarnings.predictedEarningsInTokens);
        const stakingPoolAddress = await stakingPool.getAddress();
        ...
        await expect(stakingManagement.removeStakingPlan(planId)).to.be.revertedWithCustomError(
          stakingManagement,
          'StakingPlanHasActiveStakes',
        );
      });

      it('Should not allow to remove a staking plan that does not exists', async function () {
        await expect(stakingManagement.removeStakingPlan(NON_EXISTING_PLAN_ID)).to.be.revertedWithCustomError(
          stakingManagement,
          'StakingPlanDoesNotExist',
        );
      });
      ...
      it('Should not allow to remove a staking plan if not an owner', async function () {
        await expect(stakingManagement.connect(stranger).removeStakingPlan(planId)).to.be.revertedWithCustomError(
          stakingManagement,
          'CallerIsNotAStakingManager',
        );
      });
    });
    ...
    describe('enableWithdraw', function () {
      ...
      it('Should not allow to enable withdraw if not an owner', async function () {
        await expect(stakingManagement.connect(stranger).enableWithdraw()).to.be.revertedWithCustomError(
          stakingManagement,
          'CallerIsNotAStakingManager',
        );
      });
      ...
      it('Should allow to withdraw if withdraw enabled', async function () {
        ...
        const requiredStakingPoolBalance =
          BigInt(STAKING_AMOUNT) + BigInt(stakeEstimatedEarnings.predictedEarningsInTokens);
        const stakingPoolAddress = await stakingPool.getAddress();
        await stakingToken.connect(deployer).transfer(stakingPoolAddress, requiredStakingPoolBalance);
        await stakingToken.connect(stakingPool).approve(staking.target, requiredStakingPoolBalance);
    ...
    describe('disableWithdraw', function () {
      ...
      it('Should not allow to disable withdraw if not an owner', async function () {
        await expect(stakingManagement.connect(stranger).disableWithdraw()).to.be.revertedWithCustomError(
          stakingManagement,
          'CallerIsNotAStakingManager',
        );
      });
      ...
      it('Should not allow to withdraw if withdraw disabled', async function () {
        ...
        expect(staking.connect(staker1).withdraw(stakeId)).to.be.revertedWithCustomError(
          staking,
    ...
    describe('setStakingLimits', function () {
      ...
      it('Should not allow to set the staking limits if not an owner', async function () {
        await expect(
          stakingManagement.connect(stranger).setStakingLimits(MIN_STAKING_AMOUNT, MAX_STAKING_AMOUNT),
        ).to.be.revertedWithCustomError(stakingManagement, 'CallerIsNotAStakingManager');
      });

      it('Should not allow to set the staking limits if min is greater than max', async function () {
        await expect(
          stakingManagement.setStakingLimits(MAX_STAKING_AMOUNT, MIN_STAKING_AMOUNT),
    ...
    describe('setMinimumStake', function () {
      ...
      it('Should not allow to set the minimum staking amount if not an owner', async function () {
        await expect(
          stakingManagement.connect(stranger).setMininumStake(MIN_STAKING_AMOUNT),
        ).to.be.revertedWithCustomError(stakingManagement, 'CallerIsNotAStakingManager');
      });

      it('Should not allow to set the minimum staking amount if greater than the maximum', async function () {
        await expect(stakingManagement.setMininumStake(MAX_STAKING_AMOUNT + 1)).to.be.revertedWithCustomError(
          stakingManagement,
          'MinimumStakeMustBeLessThanOrEqualToMaximumStake',
        );
      });
    });
    ...
    describe('setMaximumStake', function () {
      ...
      it('Should not allow to set the maximum staking amount if not an owner', async function () {
        await expect(
          stakingManagement.connect(stranger).setMaximumStake(MAX_STAKING_AMOUNT),
        ).to.be.revertedWithCustomError(stakingManagement, 'CallerIsNotAStakingManager');
      });

      it('Should not allow to set the maximum staking amount if less than the minimum', async function () {
        await expect(stakingManagement.setMaximumStake(MIN_STAKING_AMOUNT - 1)).to.be.revertedWithCustomError(
          stakingManagement,
          'MinimumStakeMustBeLessThanOrEqualToMaximumStake',
        );
      });
    ...
    describe('checkStakingPlanExists', function () {
      ...
      it('Should revert if the staking plan does not exists', async function () {
        await expect(stakingManagement.checkStakingPlanExists(999)).to.be.revertedWithCustomError(
          stakingManagement,
          'StakingPlanDoesNotExist',
        );
      });
    ...
    describe('setStaking', function () {
      ...
      it('Should not allow to set the staking contract if not a STAKING_MANAGER', async function () {
        await expect(stakingManagement.connect(stranger).setStaking(staking.target)).to.be.revertedWithCustomError(
          stakingManagement,
          'CallerIsNotAStakingManager',
        );
      });
